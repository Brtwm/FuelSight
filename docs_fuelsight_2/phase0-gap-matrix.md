...
| project/backend/backend-docs.md | backend architecture | implemented + worktree | `backend/app/api/v1/*`, `backend/app/services/*`, `backend/app/integrations/*`, `backend/app/pipeline/tasks.py`, `backend/app/models/rag_chunk.py` | `uv run pytest`, `backend/tests/test_rag_index_contract.py` | explain API/domains/pipeline/RAG structure | keep Phase H verified RAG baseline stable; cloud provider adapters remain Phase I |
...
| integrations-and-data-sources.md | external providers and cache strategy | partial | `backend/app/integrations/external_indicators/*`, `backend/app/integrations/news/*`, `backend/app/services/chat_retrieval.py`, target `backend/app/integrations/llm/*` | `backend/tests/test_pipeline_tasks.py`, `backend/tests/test_chat_service.py`, target provider tests | explain live/cache/manual snapshot story and verified retrieval ladder | implement NeuralDeep OpenAI-compatible profile and GigaChat alternative in Phase I |
...
| features/news-digest-chat.md | `/news` | implemented + worktree | `backend/app/services/news_service.py`, `backend/app/services/chat_service.py`, `backend/app/services/chat_retrieval.py`, `backend/app/services/rag_index_service.py`, `backend/app/api/v1/chat.py`, `frontend/src/pages/NewsPage.tsx` | `backend/tests/test_news_api.py`, `backend/tests/test_chat_api.py`, `backend/tests/test_chat_service.py`, `frontend/src/features/news/components/ChatThread.test.tsx` | analyst opens digest and asks cited verified question | stabilize Phase H confidence/verification behavior; cloud synthesis remains Phase I |
