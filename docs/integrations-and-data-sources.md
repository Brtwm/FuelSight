...
## Принцип
Все внешние интеграции работают по схеме `real provider + local cache/snapshots`.
...
- GigaChat profile:
  ...
  - adapter получает OAuth token по `GIGACHAT_AUTH_KEY`, кеширует token до expiry, вызывает `/chat/completions` и `/embeddings`;
...
- Quality layer:
  ...
  - cloud embeddings нормализуются к текущему `rag_chunks.embedding = vector(64)`, чтобы optional cloud mode не требовал migration под размерность конкретного provider;
