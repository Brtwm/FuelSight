...
| Role-aware import operations | implemented | `/import/sales`, `/import/purchases`, `/import/history`, `/api/v1/import/*` | `backend/app/api/v1/imports.py`, `frontend/src/pages/ImportPage.tsx` | `backend/tests/test_import_api.py`, role boundary tests, import component tests |
...
| Cited chat and RAG | implemented for `admin`/`analyst` chat actions | `/api/v1/chat/*`, `/news` | `backend/app/services/chat_service.py`, `backend/app/services/chat_retrieval.py`, `backend/app/services/rag_index_service.py` | chat API/service tests, RAG index tests |
...
| Demo runner and legacy technical report artifact | implemented | `scripts/run_full_demo.py` | profile-driven Docker/demo pipeline | full demo smoke and e2e smoke |

## Route Coverage

| route | current behavior |
| --- | --- |
| `/login` | local login with seeded five-role demo credentials |
| `/dashboard` | role-aware KPI/dashboard view for admin, sales, accounting, analyst and director |
| `/executive/dashboard` | director landing route for the executive dashboard view |
| `/import/sales` | sales file import for `admin` and `sales` |
| `/import/purchases` | purchase file import for `admin` and `accounting` |
| `/import/history` | role-filtered import history; admin also sees demo-history generation and diagnostics |
| `/analytics/sales` | trends, seasonality, comparisons, anomalies, URL-synced filters for `admin`, `sales`, `analyst` |
| `/analytics/margin` | procurement/margin analysis, low-margin detection, explanations for `admin`, `accounting`, `analyst`, `director` |
| `/forecast` | CatBoost-first forecast, baseline comparison, scenario deltas, quality metrics for `admin`, `sales`, `analyst`, `director` |
| `/news` | news digest/search/context for `admin`, `analyst`, `director`; backend chat actions only for `admin`, `analyst` |
| `/reports/executive` | business-facing `Управленческий отчет` for `admin`, `analyst`, `director` |
...
- The pipeline command/artifact name `build-defense-report` is legacy technical
  terminology; the implemented UI-facing business feature is `Управленческий отчет`.
