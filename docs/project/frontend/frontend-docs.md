...
- Основной стиль: `Cinematic Dark` для внутренней аналитической диспетчерской, без маркетингового лендинга и без декоративности, мешающей таблицам/графикам.
- Типографика: `Unbounded` только для бренда и крупных экранных заголовков, `IBM Plex Sans` для интерфейса, `JetBrains Mono` для чисел KPI и табличных значений.
- MUI theme использует глубокий графитовый canvas, panel/elevated surfaces, тонкую техническую сетку/noise и акценты:
  - cyan/signal для прогноза и главных линий графиков;
  - amber для риска, активных состояний и primary CTA;
  - `success` для устойчивой маржи;
  - `warning` для пограничных значений;
  - `error` для аномалий и критических алертов.
- Видимые статусные labels должны быть человекочитаемыми по-русски и располагаться внутри релевантных страниц/карточек, а не в глобальном `AppShell`: `Данные: свежие`, `Модель: проверить`, `Индикаторы: кэш`; raw `n/a`, `provider_mode`, `source_type` и похожие technical labels не выводятся пользователю.
...
## Связанные документы
- Общее видение: `@docs/project-idea.md`
- Маркетинговый контекст: `@docs/marketing/go-to-market.md`
- Общая серверная архитектура: `@docs/project/backend/backend-docs.md`
- Спецификации экранов и поведения: `@docs/features/`
- ASCII-схемы UI: `@docs/screens/`
