...
## Обзор
- **Назначение**: загрузка внутренних продаж и закупок из CSV/XLSX, а также генерация учебных данных для демонстрационного стенда.
- **Пользователь**: `admin`.
- **Точка входа**: `/import`.
...
2. Выбирает режим: загрузка продаж, загрузка закупок или генерация демо-данных.
...
|---|---|---|
| Default | Страница открыта | Табы `Продажи`, `Закупки`, `Демо-данные`, история |
...
### `GenerateDemoDataForm`
- **Расположение**: `src/features/import/components/GenerateDemoDataForm.tsx`
...
- Генератор демо-данных должен создавать согласованные продажи и закупки, пригодные для прогноза и маржи.
...
- Пользователь запускает генерацию демо-данных поверх существующих данных.

## Тестирование
- API: загрузка валидного CSV, невалидного XLSX, частичный успех, генерация демо-данных.
