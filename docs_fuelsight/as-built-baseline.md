...
| `/news` | real-provider digest/search + chat shell with citations, mobile order `digest -> chat -> search`; chat generation still MVP when LLM is off | `frontend/src/pages/NewsPage.tsx`, `backend/app/services/news_service.py`, `backend/app/services/chat_service.py` |
...
3. `news` ingest больше не использует fixture в runtime; `chat` всё ещё остаётся на `template_rag` и `LLM off -> 503` до Phase G.
