...
| Admin import operations | implemented | `/import`, `/api/v1/import/*` | `backend/app/api/v1/imports.py`, `frontend/src/pages/ImportPage.tsx`, `frontend/src/features/import/components/ImportUploadCard.tsx` | `backend/tests/test_import_api.py`, `frontend/src/pages/ImportPage.integration.test.tsx`, `frontend/src/features/import/components/ImportUploadCard.test.tsx` | admin-only diagnostics, neutral labels и drag-and-drop upload уже есть |
...
| Shared frontend primitives | implemented | shared UI layer | `frontend/src/components/common/*`, `frontend/src/app/layout/AppShell.tsx`, `frontend/src/lib/charts/chartOptions.ts` | component tests for `ChartCard`, `DataStatePanel`, `FreshnessBadgeGroup`, `SourceModeBadge`, chart option helpers | AppShell clean navigation и общий лёгкий chart helper layer уже работают |
...
| `/news` | real-provider digest/search + verified retrieval chat, confidence/verification badges, desktop sticky chat, mobile tabs `[Сводка | Поиск | Чат]` | `frontend/src/pages/NewsPage.tsx`, `frontend/src/pages/NewsPage.llmStatus.test.tsx`, `backend/app/services/news_service.py`, `backend/app/services/chat_service.py` |
...
| `corepack pnpm --filter frontend exec vitest run src/pages/NewsPage.llmStatus.test.tsx src/features/news/components/ChatThread.test.tsx` | `2 files / 13 tests passed` | P1-2 `/news` layout contract and chat interaction hardening подтверждены |
